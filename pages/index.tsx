...
import typescriptIcon from "@/public/icons/typescript-icon";
import mongoDbIcon from "@/public/icons/mongodb-icon";
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    svgs.forEach((svg, index) => {
      ...
      const positionX = (screenWidth / (svgs.length + 1)) * (index + 1);

      cubes.push(new Cube(positionX, 200, svg, ctx));
    });

    function onWindowClick(e: MouseEvent) {
      cubes.push(new Cube(e.pageX, e.pageY, svgs[iconIndexRef.current], ctx));

      if (iconIndexRef.current + 1 < svgs.length) {
        iconIndexRef.current += 1;
      } else {
        iconIndexRef.current = 0;
      }
    }
    ...
    function animate() {
      ...
      cubes.forEach((cube, index) => {
        ...
        if (cube.y > document.documentElement.scrollHeight) {
  ...
  return (
    <>
      ...
      <canvas ref={canvasRef} className=" top-0 left-0 absolute"></canvas>
    </>
  );
}
